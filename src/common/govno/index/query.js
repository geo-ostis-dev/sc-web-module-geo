...
OSM.Query = function(map) {
  ...
  function featurePrefix(feature) {
    ...
    if (tags.boundary === "administrative" && tags.admin_level) {
      prefix = "geocoder.search_osm_nominatim.admin_levels.level" + tags.admin_level;
    } else {
      var prefixes = "geocoder.search_osm_nominatim.prefix";
      var key, value;
    ...
    if (!prefix) {
      prefix = "javascripts.query." + feature.type;
    }
  ...
  function runQuery(latlng, radius, query, $section, merge, compare) {
    ...
    $section.data("ajax", $.ajax({
      ...
      success: function(results) {
        ...
        if (results.remark) {
          $("<li>")
            .text("javascripts.query.error", { server: url, error: results.remark })
            .appendTo($ul);
        }

        if ($ul.find("li").length === 0) {
          $("<li>")
            .text("javascripts.query.nothing_found")
            .appendTo($ul);
        }
      },
      error: function(xhr, status, error) {
        ...
        $("<li>")
          .text("javascripts.query." + status, { server: url, error: error })
          .appendTo($ul);
      }
    }));
  }
