...
OSM.Directions = function (map) {
  ...
  var endpoints = [
    Endpoint($("input[name='route_from']"), MAP.MARKER_GREEN),
    Endpoint($("input[name='route_to']"), MAP.MARKER_RED)
  ];
  ...
  function Endpoint(input, iconUrl) {
    ...
    endpoint.getGeocode = function() {
      ...
      $.getJSON(MAP.NOMINATIM_URL + 'search?q=' + encodeURIComponent(endpoint.value) + '&format=json', function (json) {
        endpoint.awaitingGeocode = false;
        endpoint.hasGeocode = true;
        if (json.length === 0) {
          alert('javascripts.directions.errors.no_place');
          return;
        }
  ...
  function getRoute(fitRoute, reportErrors) {
    ...
    awaitingRoute = chosenEngine.getRoute([o, d], function (err, route) {
      ...
      if (err) {
        ...
        if (reportErrors) {
          $('#sidebar_content').html('<p class="search_results_error">' + 'javascripts.directions.errors.no_route' + '</p>');
        }
      ...
      var html = '<h2><a class="geolink" href="#">' +
        '<span class="icon close"></span></a>' + 'javascripts.directions.directions' +
        '</h2><p id="routing_summary">' +
        'javascripts.directions.distance' + ': ' + formatDistance(route.distance) + '. ' +
        'javascripts.directions.time' + ': ' + formatTime(route.time) + '.';
      if (typeof route.ascend !== 'undefined' && typeof route.descend !== 'undefined') {
        html += '<br />' +
          'javascripts.directions.ascend' + ': ' + Math.round(route.ascend) + 'm. ' +
          'javascripts.directions.descend' + ': ' + Math.round(route.descend) +'m.';
      }
      ...
      $('#sidebar_content').append('<p id="routing_credit">' +
        'javascripts.directions.instructions.courtesy', {link: chosenEngine.creditline} +
        '</p>');
  ...
  engines.sort(function (a, b) {
    a = 'javascripts.directions.engines.' + a.id;
    b = 'javascripts.directions.engines.' + b.id;
  ...
  engines.forEach(function(engine, i) {
    select.append("<option value='" + i + "'>" + 'javascripts.directions.engines.' + engine.id + "</option>");
  });
