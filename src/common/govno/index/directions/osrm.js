...
function OSRMEngine() {
  ...
  return {
    ...
    _transformSteps: function(input_steps, line) {
      ...
      var transformed_steps = input_steps.map(function(step, idx) {
        ...
        if (step.name && step.ref) {
          ...
          name = 'javascripts.directions.instructions.unnamed';
          namedRoad = false;
        }

        if (step.maneuver.type.match(/rotary|roundabout/)) {
          if (step.maneuver.exit) {
            instText += template + '_with_exit', { exit: step.maneuver.exit, name: name } ;
          } else {
            instText += template + '_without_exit', { name: name } ;
          }
    ...
    getRoute: function (points, callback) {
      ...
      var req_url = MAP.OSRM_URL + encoded_coords;
