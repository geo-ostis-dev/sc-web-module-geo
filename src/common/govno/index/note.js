OSM.Note = function (map) {
  ...
  var noteIcons = {
    "new": L.icon({
      iconUrl: MAP.NEW_NOTE_MARKER,
      iconSize: [25, 40],
      iconAnchor: [12, 40]
    }),
    "open": L.icon({
      iconUrl: MAP.OPEN_NOTE_MARKER,
      iconSize: [25, 40],
      iconAnchor: [12, 40]
    }),
    "closed": L.icon({
      iconUrl: MAP.CLOSED_NOTE_MARKER,
      iconSize: [25, 40],
      iconAnchor: [12, 40]
    })
  };
  ...
  function initialize(callback) {
    ...
    content.find("textarea").on("input", function (e) {
      ...
      if ($(e.target).val() === "") {
        $(form.close).val("javascripts.notes.show.resolve");
        $(form.comment).prop("disabled", true);
      } else {
        $(form.close).val("javascripts.notes.show.comment_and_resolve");
