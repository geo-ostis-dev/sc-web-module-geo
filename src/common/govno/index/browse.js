export default function initializeBrowse(map) {
    let browseBounds;
    let selectedLayer;
    let dataLayer = map.dataLayer;
    ...
    function displayFeatureWarning(count, limit, add, cancel) {
        $('#browse_status').html(
            $("<p class='warning'></p>")
                .text("browse.start_rjs.feature_warning", {num_features: count, max_features: limit})
                .prepend(
                    $("<span class='icon close'></span>")
                        .click(cancel))
                .append(
                    $("<input type='submit'>")
                        .val('browse.start_rjs.load_data')
                        .click(add)));
    }
    ...
    function getData() {
        let bounds = map.getBounds();
        let url = "/api/" + MAP.API_VERSION + "/map?bbox=" + bounds.toBBoxString();
        ...
        dataLoader = $.ajax({
            ...
            success: function (xml) {
                ...
                let features = dataLayer.buildFeatures(xml);
